...
export default function InboxPage() {
  ...
  const loadMessages = async (reset: boolean = false) => {
    setLoading(true);
    console.log('[Inbox] Loading messages for category:', activeCategory);
    const result = await getInboxMessagesPaginated(
      { category: activeCategory, search: searchQuery },
      20,
      reset ? undefined : cursor
    );
    console.log('[Inbox] Got result:', {
  ...
  const loadCounts = async () => {
    console.log('[Inbox] Loading counts...');
    const result = await getInboxCounts();
    console.log('[Inbox] Counts:', result);
    setCounts(result);
  };
