...
import { ArrowLeft, Mail, Clock, CheckCircle, Send, Sparkles, RefreshCw, Download, Trash2, Edit2, Copy, Archive, AlertCircle, Eye, EyeOff, ChevronDown } from 'lucide-react';
...
type SyncDateRange = 'today' | 'last_3_days' | 'last_7_days' | 'last_2_weeks' | 'last_month' | 'all';
...
export default function InboxPage() {
  ...
  const [syncDateRange, setSyncDateRange] = useState<SyncDateRange>('last_7_days');
  const [showSyncMenu, setShowSyncMenu] = useState(false);

  const itemsPerPage = 50;

  const loadMessages = async (newOffset: number = 0) => {
    setLoading(true);
    const result = await getInboxMessages(itemsPerPage, newOffset);
    if (result.success && result.messages) {
      if (newOffset === 0) {
        setAllMessages(result.messages as InboxMessage[]);
      } else {
        setAllMessages(prev => [...prev, ...result.messages as InboxMessage[]]);
      }
  ...
  const filterMessages = useCallback((tab: FilterTab) => {
    ...
    switch (tab) {
      case 'unread':
        // New tab = unreplied emails (status !== 'sent' means no reply has been sent)
        filtered = allMessages.filter(m => m.status !== 'sent' && m.status !== 'archived' && m.intent !== 'spam');
        break;
      case 'spam':
        // Spam tab = emails marked as spam
        filtered = allMessages.filter(m => m.intent === 'spam');
        break;
      case 'archived':
        // Archived tab = archived emails only
        filtered = allMessages.filter(m => m.status === 'archived');
        break;
      case 'all':
      default:
        // All tab = read + unread messages (exclude archived and spam)
        filtered = allMessages.filter(m => m.status !== 'archived' && m.intent !== 'spam');
    }
  ...
  const handleSyncFromResend = async (range: SyncDateRange = syncDateRange) => {
    setSyncing(true);
    setShowSyncMenu(false);
    const result = await syncEmailsFromResend(range);
    setSyncing(false);
  ...
  const handleArchiveMessage = async (messageId: string) => {
    ...
    if (result.success) {
      const isArchiving = msg?.status !== 'archived';
      alert(isArchiving ? 'Message archived' : 'Message unarchived');
      await loadMessages();
      // Update selected message if it's open
      if (selectedMessage?.id === messageId) {
        const newStatus = isArchiving ? 'archived' : 'read';
        setSelectedMessage({ ...selectedMessage, status: newStatus });
      }
    } else {
      alert((result as any).error || 'Failed to update message');
    }
  };
  ...
  const getTabCount = (tab: FilterTab): number => {
    switch (tab) {
      case 'unread':
        // New = unreplied emails
        return allMessages.filter(m => m.status !== 'sent' && m.status !== 'archived' && m.intent !== 'spam').length;
      case 'spam':
      ...
      case 'all':
      default:
        // All = read + unread (no archived, no spam)
        return allMessages.filter(m => m.status !== 'archived' && m.intent !== 'spam').length;
    }
  };
            ...
            <div className="relative">
              ...
              {showSyncMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-300 rounded-lg shadow-lg z-50">
                  <div className="p-2">
                    <p className="text-xs font-semibold text-gray-600 px-3 py-2">Sync emails from:</p>
                    {([
                    ...
                    ] as Array<{ label: string; value: SyncDateRange }>) .map(option => (
                      <button
                        key={option.value}
                        onClick={() => handleSyncFromResend(option.value)}
                        className={`w-full text-left px-3 py-2 text-sm rounded hover:bg-gray-100 ${
                          syncDateRange === option.value ? 'bg-blue-50 text-blue-600 font-medium' : ''
                        }`}
                      >
                        {option.label}
                      </button>
          ...
          <Card className="lg:col-span-1 p-4 max-h-[800px] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold">
                ...
                {activeTab === 'unread' && 'Unreplied Messages'}
                {activeTab === 'spam' && 'Spam'}
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-xs mt-1">
                  ...
                  {activeTab === 'unread' && 'All messages have been replied to'}
                  {activeTab === 'all' && 'No messages in inbox'}
                </p>
              </div>
            ) : (
            ...
            {hasMore && messages.length > 0 && (
              <div className="mt-4 pt-4 border-t">
                <Button
                  ...
                  className="w-full"
                >
                  {loading ? 'Loading...' : `Load More (${totalMessages - offset} remaining)`}
                </Button>
              </div>
            )}
          </Card>
