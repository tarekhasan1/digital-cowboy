...
export default function CreateCampaignPage() {
  ...
  const handleSaveDraft = async () => {
    ...
    const result = await createCampaign({
      ...
      bodyPlain: body,
      recipientTags: tags.split(',').map(t => t.trim()).filter(Boolean),
    });
  ...
  const handleSendNow = async () => {
    ...
    setSending(true);
    const sendResult = await sendCampaign(createResult.campaignId | undefined );
    setSending(false);
    setLoading(false);

    if (sendResult.success) {
      alert(`Campaign sent! ${sendResult.sent} sent, ${sendResult.failed} failed`);
      router.push('/admin/email-va/campaigns');
    } else {
...
Your Agency Team`}
            ...
            <div className="bg-gray-50 p-4 rounded border">
              ...
              <p className="text-sm mb-1"><strong>Subject:</strong> {subject || '(No subject)'}</p>
            ...
            {sending && (
              <div className="bg-blue-50 p-4 rounded border border-blue-200 text-blue-800 text-sm">
                <p className="font-medium">Sending campaign...</p>
                <p className="text-xs mt-1">This may take a few minutes depending on the number of recipients.</p>
              </div>
            )}
          </div>
        </Card>
