...
// Import JWT with proper error handling
let jwt: any;
try {
  jwt = require('jsonwebtoken');
} catch (error) {
  console.warn('jsonwebtoken not available:', error);
  jwt = null;
}
...
export function generateToken(payload: { uid: string; email: string; role: string }): string {
  if (!jwt) {
    throw new Error('JWT module not available');
  }
...
export function verifyToken(token: string): { uid: string; email: string; role: string } | null {
  if (!jwt) {
    console.error('JWT module not available');
    return null;
  }
