...
export class GroqAI {
  ...
  async classifyIntent(emailContent: string): Promise<{
    ...
    const systemPrompt = `You are an email classifier for a software agency.
...
{"intent":"sales","priority":"high","confidence":0.85}`;
  ...
  async generateReply(emailContent: string, intent?: string): Promise<string> {
    const systemPrompt = `You are a professional email assistant for a software development agency.
