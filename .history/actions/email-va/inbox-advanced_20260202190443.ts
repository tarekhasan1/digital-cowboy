...
export async function getInboxMessagesPaginated(
  ...
  try {
    console.log('[getInboxMessagesPaginated] Starting with filters:', filters);
    ...
    console.log('[getInboxMessagesPaginated] Got snapshot with docs:', snapshot.docs.length);
  ...
  try {
    ...
    if (message?.resendEmailId) {
      try {
        ...
        await collections.suppressed.add({
