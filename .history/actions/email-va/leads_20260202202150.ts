  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (nameIndex !== -1 && values[nameIndex]) {
        leadData.name = values[nameIndex];
      }
      if (companyIndex !== -1 && values[companyIndex]) {
        leadData.company = values[companyIndex];
      }
      ...
      for (let j = 0; j < headers.length; j++) {
        if (j !== emailIndex && j !== nameIndex && j !== companyIndex && j !== phoneIndex && values[j]) {
          if (!leadData.customFields) leadData.customFields = {};
          leadData.customFields[rawHeaders[j]] = values[j];
        }
      }
