...
/**
 * Archive/Unarchive message - toggles between archived and read
 */
export async function archiveInboxMessage(messageId: string) {
  try {
    // Get the current message to check its status
    const messageDoc = await collections.inbox.doc(messageId).get();
    ...
    // Toggle archive: if archived, unarchive to 'read'; otherwise, archive
    const newStatus = currentMessage.status === 'archived' ? 'read' : 'archived';
