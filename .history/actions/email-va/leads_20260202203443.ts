...
/**
 * Auto-detect delimiter in CSV (comma, semicolon, tab, pipe)
 */
...
/**
 * Parse CSV line properly handling quoted fields with custom delimiter
 */
function parseCSVLine(line: string, delimiter: string = ','): string[] {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (char === '"') {
      ...
      // Delimiter outside quotes - field separator
...
/**
 * Intelligently extract name from various possible column combinations
 */
function extractName(headers: string[], values: string[]): string | undefined {
  // Priority 1: Look for single "name" column
  const nameIndex = findColumnIndex(headers, ['name', 'full name', 'contact name', 'display name']);
  if (nameIndex !== -1 && values[nameIndex]?.trim()) {
    return values[nameIndex].trim();
  }

  // Priority 2: Combine "first name" and "last name"
  const firstNameIndex = findColumnIndex(headers, ['first name', 'fname', 'first', 'given name']);
  const lastNameIndex = findColumnIndex(headers, ['last name', 'lname', 'last', 'family name', 'surname']);
  ...
  } else if (firstNameIndex !== -1 && values[firstNameIndex]?.trim()) {
    // Only first name available
    return values[firstNameIndex].trim();
  } else if (lastNameIndex !== -1 && values[lastNameIndex]?.trim()) {
    // Only last name available
    return values[lastNameIndex].trim();
  }

  // Priority 3: Look for "username" or similar
  const usernameIndex = findColumnIndex(headers, ['username', 'user', 'handle', 'login', 'account']);
  if (usernameIndex !== -1 && values[usernameIndex]?.trim()) {
    return values[usernameIndex].trim();
  }

  // Priority 4: Look for "contact" or generic identifier
  const contactIndex = findColumnIndex(headers, ['contact', 'person', 'individual']);
  if (contactIndex !== -1 && values[contactIndex]?.trim()) {
    return values[contactIndex].trim();
  }
...
export async function importLeads(params: ImportLeadsParams) {
  try {
    ...
    if (emailIndex === -1) {
      return { 
        success: false, 
        error: `Email column not found. Available columns: ${rawHeaders.join(', ')}. Detected delimiter: ${delimiter === ';' ? 'semicolon' : delimiter === '\t' ? 'tab' : delimiter === '|' ? 'pipe' : 'comma'}`, 
        imported: 0, 
        skipped: 0 
      };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Map detected fields with smart extraction
      const extractedName = extractName(headers, values);
      if (extractedName) {
      ...
      const usedColumnIndexes = new Set([emailIndex, companyIndex, phoneIndex]);
      
      // Also add name-related columns to used set
      const firstNameIndex = findColumnIndex(headers, ['first name', 'fname', 'first', 'given name']);
      const lastNameIndex = findColumnIndex(headers, ['last name', 'lname', 'last', 'family name', 'surname']);
      const nameIndex = findColumnIndex(headers, ['name', 'full name', 'contact name', 'display name']);
      const usernameIndex = findColumnIndex(headers, ['username', 'user', 'handle', 'login', 'account']);
      
      if (firstNameIndex !== -1) usedColumnIndexes.add(firstNameIndex);
      if (lastNameIndex !== -1) usedColumnIndexes.add(lastNameIndex);
      if (nameIndex !== -1) usedColumnIndexes.add(nameIndex);
      if (usernameIndex !== -1) usedColumnIndexes.add(usernameIndex);

      for (let j = 0; j < headers.length; j++) {
        const value = values[j]?.trim();
        if (!usedColumnIndexes.has(j) && value) {
    ...
    return {
      ...
      detectedColumns: {
        ...
        name: 'Smart extraction (Name, First Name + Last Name, or Username)',
