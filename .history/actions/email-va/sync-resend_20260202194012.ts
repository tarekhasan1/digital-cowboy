...
export type SyncDateRange = 'today' | 'last_3_days' | 'last_7_days' | 'last_2_weeks' | 'last_month' | 'all';

/**
 * Helper: Get date range for sync
 */
function getDateRangeFromFilter(range: SyncDateRange): { startDate: Date; label: string } {
  const now = new Date();
  let startDate: Date;
  let label: string;
 ...
 * @param dateRange - Filter emails by date range (default: last_7_days)
 */
export async function syncEmailsFromResend(dateRange: SyncDateRange = 'last_7_days') {
  try {
    const { startDate, label } = getDateRangeFromFilter(dateRange);
    console.log(`🔄 Syncing received emails from Resend (${label})...`);
    ...
    let filteredEmails = result.emails.filter((email: any) => {
      const emailDate = email.created_at ? new Date(email.created_at) : new Date();
      return emailDate >= startDate;
    });

    console.log(`📧 Found ${result.emails.length} received emails in Resend, ${filteredEmails.length} in ${label}`);
