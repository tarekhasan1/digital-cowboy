...
/**
 * Delete a single message (moved to spam) - DEPRECATED: Use inbox-advanced.ts
 */
export async function deleteInboxMessage(messageId: string) {
  try {
    ...
    // Move to spam instead of soft delete
    await collections.inbox.doc(messageId).update({
      category: 'spam',
      updatedAt: new Date(),
    });

    console.log(`🚫 Message ${messageId} moved to spam`);
    return { success: true };
  } catch (error: any) {
...
/**
 * Bulk delete multiple messages - DEPRECATED: Use inbox-advanced.ts
 */
export async function bulkDeleteInboxMessages(messageIds: string[]) {
  try {
    ...
    console.log(`🚫 Moved ${messageIds.length} messages to spam`);
